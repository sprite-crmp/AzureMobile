...
void InitBASSFuncs()
{
	...
	handle = dlopen("/data/data/ru.azure.games/lib/libbass.so", 3);
#else
	handle = dlopen("/data/data/ru.azure.games/lib/libbass.so", 3);
#endif
	if (!handle)
...
bool ProcessVoiceCommands(const char* str)
{
	...
	if (strstr(str, "/cameditgui"))
	{
		...
		{
			int reytiz = 0;
			g_pJavaWrapper->HideCameditgui(clickhide);
			pNetGame->SendCameditgui(reytiz);
			clickhide = 1;
			return true;
		}
		if(clickhide == 1)
...
void CChatWindow::AddDebugMessage(char* szFormat, ...)
{
    ...
    FilterInvalidChars(tmp_buf);
    g_pJavaWrapper->AddChatMessage(tmp_buf, m_dwDebugColor);
...
void CChatWindow::AddDebugMessageNonFormatted(char* szStr)
{
    ...
    g_pJavaWrapper->AddChatMessage(szStr, m_dwDebugColor);
