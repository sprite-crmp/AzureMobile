...
stFile* NvFOpen_hook(const char* r0, const char* r1, int r2, int r3)
{
	...
	{
		sprintf(path, "%sSAMP/azuremain.scm", g_pszStorage);
...
void ShowHud() 
{
    *(uint8_t*)(g_libGTASA+0x7165E8) = 0;
	...
	/*CLocalPlayer *pLocalPlayer = pNetGame->GetPlayerPool()->GetLocalPlayer(); фикс Java худа на новых телефонах
	if(pGame) 
	{
...
void MobileMenu__Render_hook(uintptr_t thiz)
{
	MobileMenu__Render(thiz);
	 const char/*jstring*/ *ponel = OBFUSCATE("Azure Mobile");
...
{	
	NvUtilInit();
	*(char**)(g_libGTASA + 0x5D1608) = "/storage/emulated/0/Android/data/ru.azure.games/files/";
}
