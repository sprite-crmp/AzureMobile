...
{	
	...
	else
	{
		...
		if(pPlayer && pCamera)
		{
			...
			pCamera->SetPosition(1746.3688f, 1729.5567f, 20.3171f, 0.0f, 0.0f, 0.0f);
			pCamera->LookAtPoint(1746.3688f, 1729.5567f, 20.3171f, 2);
			pGame->SetWorldWeather(m_byteWeather);
			pGame->DisplayWidgets(false);
		}
		
		// выдача машины при выключенном сервере
		/*CPlayerPed *pPlayer = pGame->FindPlayerPed();
		CCamera *pCamera = pGame->GetCamera();
                                    CPlayerPool *pPlayerPool = pNetGame->GetPlayerPool();

		if(!carCreated && pPlayer && pCamera)
		{                                                     
                                                      pCamera->Restore(); // убираем камеру после инициализации
  	                                    pCamera->SetBehindPlayer();
			
                                                      if(pPlayer->IsInVehicle()) // fix scm
				pPlayer->RemoveFromVehicleAndPutAt(198.5111,727.7470,12.0005);
			else
				pPlayer->TeleportTo(198.5111,727.7470,12.0005);
	                                                      ...
	                                                      pGame->DisableTrainTraffic(); // крашфикс
                                                                        ...
                                                                        pPlayer->SetModelIndex(258); // даем скин
			...
			pGame->ToggleRadar(false); // скрываем радар
		                               ...
		                               case 0: // BMW M5 F90
		                               {
                                                                                 CVehicle *pVehicleNew = new CVehicle(466, 204.6150,732.7288,12.0005, 0, false);
                                                                                 //pPlayer->PutDirectlyInVehicle(pVehicleNew, 0);
			                           break;
		                               }	
									   case 1: // URUS
		                               {
                                                                                 CVehicle *pVehicleNew = new CVehicle(400, 204.6150,732.7288,12.0005, 0, false);
                                                                                 //pPlayer->PutDirectlyInVehicle(pVehicleNew, 0);
			                           break;
		                               }
		                               case 2: // LAMBO 
		                               {
                                                                                 CVehicle *pVehicleNew = new CVehicle(415, 204.6150,732.7288,12.0005, 0, false);
                                                                                 //pPlayer->PutDirectlyInVehicle(pVehicleNew, 0);
			                           break;
		                               }
		                               case 3: // MERS I63 AMG
		                               {
                                                                                 CVehicle *pVehicleNew = new CVehicle(429, 204.6150,732.7288,12.0005, 0, false);
                                                                                 //pPlayer->PutDirectlyInVehicle(pVehicleNew, 0);
			                           break;
		                               }
		                               case 4: // BMW I8
		                               {
                                                                                 CVehicle *pVehicleNew = new CVehicle(494, 204.6150,732.7288,12.0005, 0, false);
                                                                                 //pPlayer->PutDirectlyInVehicle(pVehicleNew, 0);
			                           break;
		                               }
		                               case 5: // RANGE ROVER
		                               {
                                                                                 CVehicle *pVehicleNew = new CVehicle(490, 204.6150,732.7288,12.0005, 0, false);
                                                                                 //pPlayer->PutDirectlyInVehicle(pVehicleNew, 0);
			                           break;
		                               }
		                               case 6: // NISSAN GTR
		                               {
                                                                                 CVehicle *pVehicleNew = new CVehicle(502, 204.6150,732.7288,12.0005, 0, false);
                                                                                 //pPlayer->PutDirectlyInVehicle(pVehicleNew, 0);
			                           break;
		                               }
		                               case 7: // ф90
		                               {
                                                                                 CVehicle *pVehicleNew = new CVehicle(551, 204.6150,732.7288,12.0005, 0, false);
                                                                                 //pPlayer->PutDirectlyInVehicle(pVehicleNew, 0);
			                           break;
		                               }
		                               case 8: // буггати ла ноир
		                               {
                                                                                 CVehicle *pVehicleNew = new CVehicle(555, 204.6150,732.7288,12.0005, 0, false);
                                                                                 //pPlayer->PutDirectlyInVehicle(pVehicleNew, 0);
			                           break;
		                               }
		                               case 9: // банана
		                               {
                                                                                 CVehicle *pVehicleNew = new CVehicle(560, 204.6150,732.7288,12.0005, 0, false);
                                                                                 //pPlayer->PutDirectlyInVehicle(pVehicleNew, 0);
			                           break;
		                               }
		                               case 10: // акула
		                               {
                                                                                 CVehicle *pVehicleNew = new CVehicle(546, 204.6150,732.7288,12.0005, 0, false);
                                                                                 //pPlayer->PutDirectlyInVehicle(pVehicleNew, 0);
			                           break;
		                               }
		                               case 11: // урус
		                               {
                                                                                 CVehicle *pVehicleNew = new CVehicle(579, 204.6150,732.7288,12.0005, 0, false);
                                                                                 //pPlayer->PutDirectlyInVehicle(pVehicleNew, 0);
			                           break;
		                               }
                                                       }    
		...
		(GetTickCount() - m_dwLastConnectAttempt) > 3000)
	{
		if(pChatWindow) pChatWindow->AddDebugMessageNonFormatted(CLocalisation::GetMessage(E_MSG::CONNECTING));
                                    ...
                                    static bool notifyCreated = false;
                                    if(!notifyCreated) {
                                                //pChatWindow->AddDebugMessageNonFormatted("{ffff00}| {ffffff}The local server is loaded"); 
                                                //pChatWindow->AddDebugMessageNonFormatted("{56D12A}| {ffffff}If the server is closed, you can take a ride in the car");
		            notifyCreated = true;
                                    }
	...
	while(pkt = m_pRakClient->Receive())
	{
		...
		switch(packetIdentifier)
		{
			...
			case ID_CONNECTION_BANNED:
                        //g_pJavaWrapper->ShowNotification(4, "Произошла ошибка, возможно вы заблокированы", 7, "", ">>");
			SetGameState(GAMESTATE_WAIT_CONNECT);
			break;

			case ID_INVALID_PASSWORD:
			g_pJavaWrapper->ShowNotification(4, "Неверный пароль для подключения к серверу!", 7, "", ">>");
			m_pRakClient->Disconnect(0);
			break;
...
#define RPC_PHONE	0x49
#define RPC_PROMOUTER	0x50
#define RPC_ADOBNOVA	0x51
#define RPC_STARLINE	0x52
#define RPC_BTNPHONE	0x53
#define RPC_SBER	0x54
#define RPC_SBERREG	0x55
#define RPC_CAMEDITGUI	0x56
#define RPC_DONATE	0x57
#define RPC_HUDSTATA	0x58
#define RPC_WANTEDHUD	0x59
#define RPC_RUDA	0x60
#define RPC_RUDASKLAD	0x61
#define RPC_UPDATEFUEL	0x62
#define RPC_LOCK	0x63
#define RPC_LIGHT	0x64
#define RPC_ENGINE	0x65
#define RPC_LOADING	0x66
#define RPC_LOADINGJAVA	0x67
#define RPC_SHOWDIALOG	0x68
#define RPC_CLOSEDIALOG	0x69
#define RPC_CHECK	0x70
#define RPC_CHECKOKAY	0x71
#define RPC_DATETIME	0x72
#define RPC_RECONNECT	0x73
#define RPC_CALLNETGAME	0x74
#define RPC_PRINTS	0x75
...
{	
	...
	case RPC_CALLNETGAME:
	{
		...
		if(action == 1)
		{
			pNetGame->SendChatCommand("/cameditgui");
		}
		return;
	}
	case RPC_RECONNECT:
	...
	case RPC_DATETIME:
	{
		...
		uint16_t lenDatetime;
		bs.Read(lenDatetime);

		memset(datetime, 0, sizeof(datetime));
		memset(szBuff, 0, sizeof(szBuff));

		bs.Read(szBuff, lenDatetime);
		cp1251_to_utf8(datetime, szBuff);

		int action;
		bs.Read(action);

		g_pJavaWrapper->DateTimeUpd(datetime, action);
		return;
	}
	case RPC_CHECK:
	{
		...
		g_pJavaWrapper->CheckVersion(reytiz);
		return;
	}
	case RPC_CLOSEDIALOG:
	{
		g_pJavaWrapper->CloseDialog();
		return;
	}
	case RPC_SHOWDIALOG:
	{
		...
		bs.Read(szBuff, lenPlayer);
		cp1251_to_utf8(player, szBuff);

		//текст диалога
		uint16_t lenPlayerDialog;
		bs.Read(lenPlayerDialog);

		memset(playerdialog, 0, sizeof(playerdialog));
		memset(szBuff, 0, sizeof(szBuff));

		bs.Read(szBuff, lenPlayerDialog);
		cp1251_to_utf8(playerdialog, szBuff);
		...
		g_pJavaWrapper->ShowQuestDialog(player, playerdialog, size, time);
		return;
	}
	case RPC_LOADING:
	{
		...
		g_pJavaWrapper->ShowLoading(loadingbar);
		return;
	}
	case RPC_UPDATEFUEL:
	{
		...
		g_pJavaWrapper->UpdateFuel(fuelbar);
		return;
	}
	case RPC_RUDASKLAD:
	{
		int rudaskladvar;
		bs.Read(rudaskladvar);

		g_pJavaWrapper->ShowRudasklad(rudaskladvar);
		return;
	}
	case RPC_RUDA:
	{
		...
		g_pJavaWrapper->ShowRuda(rudavar);
		return;
	}
	case RPC_WANTEDHUD:
	{
		...
		g_pJavaWrapper->ShowWantedhud(wantedhudd);
		return;
	}
	case RPC_HUDSTATA:
	{
		...
		g_pJavaWrapper->ShowHudstata(lvlhudd);
		return;
	}
	case RPC_DONATE:
	{
		...
		g_pJavaWrapper->ShowDonate(reytiz);
		return;
	}
	case RPC_SBERREG:
	{
		...
		g_pJavaWrapper->ShowSberreg(reytiz);
		return;
	}
	case RPC_ADOBNOVA:
	{
		...
		g_pJavaWrapper->ShowAdobnova(text);
		return;
	}
	case RPC_PROMOUTER:
	{
		...
		g_pJavaWrapper->ShowPromouter(reytiz);
		return;
	}
	case RPC_PHONE:
	{
		...
		g_pJavaWrapper->ShowPhone(reytiz);
		return;
	}
	case RPC_GUI_ALT:
	{
...
void CNetGame::SendPrints(int action)
{
	RakNet::BitStream bsSend;
        bsSend.Write((uint8_t)  0x75);
	bsSend.Write((uint32_t) RPC_PRINTS);
	bsSend.Write((int) action);

	pNetGame->GetRakClient()->Send(&bsSend, MEDIUM_PRIORITY, RELIABLE, 0);
}

void CNetGame::CheckVersionOkay(int vercache, int verapk)
{
	RakNet::BitStream bsSend;
        bsSend.Write((uint8_t)  0x71);
	bsSend.Write((uint32_t) RPC_CHECKOKAY);
	bsSend.Write((int) vercache);
	bsSend.Write((int) verapk);

	pNetGame->GetRakClient()->Send(&bsSend, MEDIUM_PRIORITY, RELIABLE, 0);
}

void CNetGame::SendLoadingJava(int reytiz)
{
	RakNet::BitStream bsSend;
        bsSend.Write((uint8_t)  0x67);
	bsSend.Write((uint32_t) RPC_LOADINGJAVA);
	bsSend.Write((int) reytiz);

	pNetGame->GetRakClient()->Send(&bsSend, MEDIUM_PRIORITY, RELIABLE, 0);
}
void CNetGame::SendLock(int reytiz)
{
	RakNet::BitStream bsSend;
	bsSend.Write((uint8_t)  0x63);
        bsSend.Write((uint32_t) RPC_LOCK);
	bsSend.Write((int) reytiz);

	pNetGame->GetRakClient()->Send(&bsSend, MEDIUM_PRIORITY, RELIABLE, 0);
}
void CNetGame::SendLight(int reytiz)
{
	RakNet::BitStream bsSend;
        bsSend.Write((uint8_t)  0x64);
        bsSend.Write((uint32_t) RPC_LIGHT);
	bsSend.Write((int) reytiz);

	pNetGame->GetRakClient()->Send(&bsSend, MEDIUM_PRIORITY, RELIABLE, 0);
}
void CNetGame::SendEngine(int reytiz)
{
	RakNet::BitStream bsSend;
        bsSend.Write((uint8_t)  0x65);
        bsSend.Write((uint32_t) RPC_ENGINE);
	bsSend.Write((int) reytiz);

	pNetGame->GetRakClient()->Send(&bsSend, MEDIUM_PRIORITY, RELIABLE, 0);
}

void CNetGame::SendCameditgui(int reytiz)
{
	RakNet::BitStream bsSend;
        bsSend.Write((uint8_t)  0x56);
        bsSend.Write((uint32_t) RPC_CAMEDITGUI);
	bsSend.Write((int) reytiz);

        pNetGame->GetRakClient()->Send(&bsSend, MEDIUM_PRIORITY, RELIABLE, 0);
}

void CNetGame::SendBtnphone(int reytiz)
{
	RakNet::BitStream bsSend;
        bsSend.Write((uint8_t)  0x53);
        bsSend.Write((uint32_t) RPC_BTNPHONE);
	bsSend.Write((int) reytiz);

        pNetGame->GetRakClient()->Send(&bsSend, MEDIUM_PRIORITY, RELIABLE, 0);
}

void CNetGame::SendSber(int reytiz)
{
	RakNet::BitStream bsSend;
        bsSend.Write((uint8_t)  0x54);
        bsSend.Write((uint32_t) RPC_SBER);
	bsSend.Write((int) reytiz);

	pNetGame->GetRakClient()->Send(&bsSend, MEDIUM_PRIORITY, RELIABLE, 0);
}

void CNetGame::SendStarline(int reytiz)
{
	RakNet::BitStream bsSend;
        bsSend.Write((uint8_t)  0x52);
        bsSend.Write((uint32_t) RPC_STARLINE);
	bsSend.Write((int) reytiz);
