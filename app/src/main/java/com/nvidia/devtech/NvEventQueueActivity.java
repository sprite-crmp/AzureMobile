...
import ru.azure.games.core.DialogClientSettings;
import ru.azure.games.gui.Adobnova;
import ru.azure.games.gui.CheckVersion;
import ru.azure.games.gui.DateTimeHud;
import ru.azure.games.gui.Donate;
import ru.azure.games.gui.HudManager;
import ru.azure.games.gui.HudStata;
import ru.azure.games.gui.LoadingBackground;
import ru.azure.games.gui.Phone;
import ru.azure.games.gui.Promouter;
import ru.azure.games.gui.QuestDialog;
import ru.azure.games.gui.Ruda;
import ru.azure.games.gui.Rudasklad;
import ru.azure.games.gui.Sberreg;
...
import ru.azure.games.gui.Notification;
import ru.azure.games.gui.Menu;
...
public abstract class NvEventQueueActivity
    ...
    private Promouter mPromouter = null;
    private Adobnova mAdobnova = null;
    private Sberreg mSberreg = null;
    private Donate mDonate= null;
    private HudStata mHudstata = null;
    private HudManager mWantedhud = null;
    private Ruda mRuda = null;
    private Rudasklad mRudasklad = null;
    private Speedometer mUpdatefuel = null;
    private HudManager mHidecameditgui = null;
    private LoadingBackground mLoading = null;
    private QuestDialog mQuestDialog = null;
    private QuestDialog mCloseDialog = null;
    private CheckVersion mCheck = null;
    private DateTimeHud mDateTime = null;
    ...
    public native void sendReytizClick(int reytiz, int action);
    public native void sendBtnphoneClick(int reytiz, int action);
    public native void sendSberClick(int reytiz, int action);
    public native void sendCameditguiClick(int reytiz, int action);
    public native void sendLockClick(int reytiz, int action);
    public native void sendLightClick(int reytiz, int action);
    public native void sendEngineClick(int reytiz, int action);
    public native void sendLoadingClick(int reytiz, int action);
    public native void checkversion(int vercache, int verapk);
    public native void SpeedStop(int stopped);
    ...
    public void onHeightChanged(int orientation, int height)
    {
        ...
        Dialog dialog = InterfacesManager.getInterfacesManager().getDialogManager();
        if (dialog != null) {
    ...
    protected boolean systemInit()
    {
        ...
        mFrontUILayout = (FrameLayout) findViewById(R.id.front_ui_layout_edgar);
        ...
        mPromouter = new Promouter(this);
        mAdobnova = new Adobnova(this);
        mSberreg = new Sberreg(this);
        mDonate = new Donate(this);
        mHudstata = new HudStata(this);
        mWantedhud = new HudManager(this);
        mRuda = new Ruda(this);
        mRudasklad = new Rudasklad(this);
        mUpdatefuel = new Speedometer(this);
        mHidecameditgui = new HudManager(this);
        mLoading = new LoadingBackground(this);
        mQuestDialog = new QuestDialog(this);
        mCloseDialog = new QuestDialog(this);
        mCheck = new CheckVersion(this);
        mDateTime = new DateTimeHud(this);
    ...
    public void setScreenDecay(boolean z6, int i10) {
    ...
    public void callLauncherActivity()
    {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Intent launchIntent = getPackageManager().getLaunchIntentForPackage("ru.azuremobile.launcher");
    ...
    public void showHudstata(int lvlhudd) { runOnUiThread(() -> { mHudstata.show(lvlhudd); }); }
    public void showWantedhud(int wantedhudd) { runOnUiThread(() -> { mWantedhud.UpdateWanted(wantedhudd); }); }
    ...
    public void showSpeedCall() { runOnUiThread(() -> { mSpeedometer.ShowSpeedCall(); }); }
    public void hideSpeedCall() { runOnUiThread(() -> { mSpeedometer.HideSpeedCall(); }); }
    ...
    public void showPhone(int reytiz) { runOnUiThread(() -> { mPhone.show(reytiz); } ); }
    public void showPromouter(int reytiz) { runOnUiThread(() -> { mPromouter.show(reytiz); } ); }
    public void showAdobnova(String text) { runOnUiThread(() -> { mAdobnova.show(text); } ); }
    public void showSberreg(int reytiz) { runOnUiThread(() -> { mSberreg.show(reytiz); } ); }
    public void showDonate(int reytiz) { runOnUiThread(() -> { mDonate.show(reytiz); } ); }
    public void showRuda(int rudavar) { runOnUiThread(() -> { mRuda.show(rudavar); } ); }
    public void showRudasklad(int rudaskladvar) { runOnUiThread(() -> { mRudasklad.Show(rudaskladvar); } ); }
    public void updateFuel(int fuelbar) { runOnUiThread(() -> { mUpdatefuel.UpdateFuel(fuelbar); } ); }
    public void hideCameditgui(int clickhide) { runOnUiThread(() -> { mHidecameditgui.HideCameditgui(clickhide); } ); }
    public void showLoading(int loadingbar) { runOnUiThread(() -> { mLoading.show(loadingbar); } ); }
    public void showQuestDialog(String player, String playerdialog, int size, int time) { runOnUiThread(() -> { mQuestDialog.show(player, playerdialog, size, time); } ); }
    public void closeDialog() { runOnUiThread(() -> { mCloseDialog.close(); } ); }
    public void checkVersion(int reytiz) { runOnUiThread(() -> { mCheck.check(reytiz); } ); }
    public void dateTime(String datetime, int action) { runOnUiThread(() -> { mDateTime.show(datetime, action); } ); }
    public void openTime(int action) { runOnUiThread(() -> { mDateTime.opentime(action); } ); }
