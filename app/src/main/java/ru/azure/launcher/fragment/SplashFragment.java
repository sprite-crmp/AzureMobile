...
import ru.azure.launcher.other.Interface;
        ...
        scall.enqueue(new Callback<List<Servers>>() {
            ...
            public void onResponse(Call<List<Servers>> call, Response<List<Servers>> response) {
                ...
                for (Servers server : servers) {
                    Lists.slist.add(new Servers(server.getColor(), server.getname(), server.getOnline(), server.getmaxOnline(), server.getRecommend(), server.getHost(), server.getPort(), server.getId(), server.getTest()));
                }
            }
...
//splash_logo.animate().alpha(0.0f).setDuration(150); /*k5.c.j(this.f8704b, 0.0f, 150);*/
}
